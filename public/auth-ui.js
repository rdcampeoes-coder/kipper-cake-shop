(()=>{
  ...
  function showForgot(prefill=''){
    ...
    el.querySelector('#sendReset').onclick=async()=>{const email=el.querySelector('#forgotEmail').value.trim();if(!email)return message('Indica o teu email.');message('A enviar…');const {error}=await client.auth.resetPasswordForEmail(email,{redirectTo:appUrl()});if(error)return message(error.message);showRecoverySent(email)};
