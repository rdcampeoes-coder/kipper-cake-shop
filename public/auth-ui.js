(()=>{
  ...
  async function consumeSupabaseLink(raw,expectedType){
    const text=String(raw||'').trim();
    if(!text)throw new Error('Cola o link completo que recebeste no email.');
    let u;
    try{u=new URL(text)}catch{throw new Error('O link não parece válido. Copia o endereço completo do email.')}
    const qp=new URLSearchParams(u.search);
    const hp=new URLSearchParams((u.hash||'').replace(/^#/,''));
    const access=hp.get('access_token'),refresh=hp.get('refresh_token');
    if(access&&refresh){
      const {data,error}=await client.auth.setSession({access_token:access,refresh_token:refresh});
      if(error)throw error; session=data.session; return {type:hp.get('type')||expectedType||'',session};
    }
    const tokenHash=qp.get('token_hash')||qp.get('token');
    const type=qp.get('type')||expectedType;
    if(!tokenHash||!type)throw new Error('Não encontrei o código de validação nesse link. Copia o link diretamente do botão do email.');
  ...
  function showRegister(){
    ...
    el.querySelector('#finishRegister').onclick=async()=>{const email=el.querySelector('#regEmail').value.trim(),password=el.querySelector('#regPass').value,password2=el.querySelector('#regPass2').value;if(!email)return message('Indica o teu email.');if(password.length<6)return message('A palavra-passe precisa de pelo menos 6 caracteres.');if(password!==password2)return message('As palavras-passe não coincidem.');message('A criar conta…');const {data,error}=await client.auth.signUp({email,password,options:{data:{plan:selectedPlan}}});if(error)return message(error.message);if(!data.session)return showConfirmHelp(email);session=data.session;await afterLogin()};
    el.querySelector('#changePlan').onclick=showPlans;el.querySelector('#backLogin').onclick=()=>showLogin();
  }

  function showConfirmHelp(email){
    const el=mount(`<h1>Confirma o teu email</h1><p>Enviámos uma mensagem para <strong>${esc(email)}</strong>.</p><p>O botão do email pode abrir uma página <strong>localhost</strong>. A confirmação é feita antes dessa página abrir, por isso podes voltar aqui e entrar normalmente.</p><div class="auth-note">Se preferires não abrir o localhost, copia o endereço do botão “Confirm email address” e cola-o aqui. A aplicação valida-o diretamente.</div><textarea id="confirmLink" placeholder="Cola aqui o link completo do email"></textarea><div id="authMsg" class="auth-msg"></div><button id="verifyConfirm" class="auth-primary">Validar link aqui</button><button id="confirmedLogin" class="auth-secondary">Já confirmei — voltar ao login</button>`);
    el.querySelector('#verifyConfirm').onclick=async()=>{try{message('A validar…');await consumeSupabaseLink(el.querySelector('#confirmLink').value,'email');message('Email confirmado. A entrar…');setTimeout(afterLogin,500)}catch(e){message(e.message)}};
    el.querySelector('#confirmedLogin').onclick=()=>showLogin(email);
  }

  function showForgot(prefill=''){
    const el=mount(`<h1>Redefinir palavra-passe</h1><p>Indica o email da tua conta. Vamos enviar-te um link de recuperação.</p><input id="forgotEmail" type="email" placeholder="Email" autocomplete="email" value="${esc(prefill)}"><div id="authMsg" class="auth-msg"></div><button id="sendReset" class="auth-primary">Enviar link de recuperação</button><button id="backLogin" class="auth-link">Voltar ao login</button>`);
    el.querySelector('#sendReset').onclick=async()=>{const email=el.querySelector('#forgotEmail').value.trim();if(!email)return message('Indica o teu email.');message('A enviar…');const {error}=await client.auth.resetPasswordForEmail(email);if(error)return message(error.message);showRecoveryPaste(email)};
    el.querySelector('#backLogin').onclick=()=>showLogin(el.querySelector('#forgotEmail').value.trim());
  }

  function showRecoveryPaste(email){
    const el=mount(`<h1>Email enviado</h1><p>Abre o email de recuperação. Se o botão te mandar para <strong>localhost</strong>, copia o endereço completo do botão ou o endereço que ficou no navegador e cola-o aqui.</p><textarea id="recoveryLink" placeholder="Cola aqui o link de recuperação"></textarea><div id="authMsg" class="auth-msg"></div><button id="useRecoveryLink" class="auth-primary">Validar e escolher nova palavra-passe</button><button id="backLogin" class="auth-link">Voltar ao login</button>`);
    el.querySelector('#useRecoveryLink').onclick=async()=>{try{message('A validar…');await consumeSupabaseLink(el.querySelector('#recoveryLink').value,'recovery');showNewPassword()}catch(e){message(e.message)}};
    el.querySelector('#backLogin').onclick=()=>showLogin(email);
  }

  function showNewPassword(){
    const el=mount(`<h1>Nova palavra-passe</h1><p>Escolhe uma nova palavra-passe para a tua conta.</p><input id="newPass" type="password" placeholder="Nova palavra-passe" autocomplete="new-password"><input id="newPass2" type="password" placeholder="Repetir nova palavra-passe" autocomplete="new-password"><div id="authMsg" class="auth-msg"></div><button id="saveNewPass" class="auth-primary">Guardar nova palavra-passe</button>`);
